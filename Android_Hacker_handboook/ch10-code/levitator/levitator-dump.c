...
main(int argc, char **argv)
{
	...
	if (fd2 == -1) {
		printf("[-] failed opening dump file, aborting!\n");
		exit(1);
	}
	
	if (write(fd2, dump, DUMP_SIZE) != DUMP_SIZE) { 
		printf("[-] failed to write dump data, aborting!\n");
		exit(1);
	}
